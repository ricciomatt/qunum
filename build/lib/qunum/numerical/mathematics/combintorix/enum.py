...
class YieldArgCombos:
    ...
    def __iter__(self)->Generator[torch.Tensor, None, None]:
        def yield_arg_combos_inner(args:tuple[tuple[Iterable[Any]]], cix:torch.Tensor, idx:torch.Tensor, ret_tensor:bool = True)->Generator[tuple[Any], None, None]:
            fst:bool = True
            while True:
                if(fst):
                    fst = False
                ...
                if not (torch.all(cix == idx-1)) :
                    ...
                    break
        return yield_arg_combos_inner(self.args, self.cix.clone(), self.idx.clone(), self.ret_tensor)
    
    def __list__(self,)->list[torch.Tensor|Any]:
        return list(iter(self))
    
    def __tuple__(self)->tuple[torch.Tensor|Any]:
        return tuple(iter(self))

    def __set__(self):
        return set(iter(self))

    def __call__(self)->torch.Tensor|np.ndarray[Any]|list[Any]:
        a = list(self)
        try:
            if(self.ret_tensor):
