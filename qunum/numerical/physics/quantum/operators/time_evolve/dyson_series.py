...
from .....mathematics.tensors import eye
import torch 
...
class DysonSeriesGenerator:
    ...
    def getUSeries(self, t:torch.Tensor)->TQobj:
        ...
        U = TQobj(dyson_series(H, self.dt, order=self.order, L = self.IntegratorCoefficents), meta = H._metadata)
        del H
        return U 
...
def dyson_series(H:torch.Tensor, dt:float, order:int, L:torch.Tensor, hbar:float)->torch.Tensor:
    H*= (-1j/hbar)
    U = torch.empty((order, *H.shape), dtype=H.dtype, device=H.device)
    U[0,:] = torch.eye(*H.shape[1:], dtype = H.dtype, device=H.device)
