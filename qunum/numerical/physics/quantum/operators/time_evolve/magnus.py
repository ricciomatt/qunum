...
class MagnusGenerator:
    ...
    def __call__(self, t:torch.Tensor)->TQobj:
        O = self.getOmega(t)
        return O.sum(dim = 0).expm()
    
    def getOmega(self, t)->TQobj:
        H = self.Hamiltonian(t)
        O = TQobj(expansion(H = H.to_tensor(), Bk=self.BrenouliCoefficents, order=self.order, dx = (t[1]-t[0]), L = self.InterpolationCoefficents, hbar=self.hbar), meta=H._metadata)
        del H
        return O 
...
def get_expansion_tex( 
    ...
    st = {'\\Omega_{0} ':'0', "\\Omega_{1}":" \\int_{t_0}^{t} dt' H(t')"}
    S = {}
    if(order >= 2):
        for k in range(2, order+1):
            n = k-1
            ...
            for i in range(1, k):
                j = i-1
                if(i == 1):
                    temp.append('\\frac{B_{'+str(i)+'}}{{'+str(i)+'}!} [\\Omega_{'+str(n)+'}, (-iH)]')
                    S[n,j] = '[\\Omega_{'+str(n)+'}, (-iH)]'
                elif(i == n):
                    temp.append('\\frac{B_{'+str(i)+'}}{{'+str(i)+'}!} ad^{'+str(j+1)+'}_{\\Omega_{1}} (-iH)')
                    S[(n,j)] = 'ad^{'+str(j+1)+'}_{\\Omega_{1}} (-iH)'
                else:
                    t = []
                    S[(n,j)] = ''
                    for m in range(1, n-j):
                        t.append('[\\Omega_{'+str(n-m)+'}, S_{'+str(n-m)+'}^{'+str(j)+'}]')
                    S[(n,j)] = '+'.join(t)
                    temp.append('\\frac{B_{'+str(i)+'}}{{'+str(i)+'}!}\\left('+'+'.join(t)+'\\right)')
            st[f'\\Omega_{k}'] = ' + '.join(temp)
