...
class LazyJordanWigner:
    ...
    def __call__(self, nix:Iterable[int]|int|None = None, generator:bool=False) -> SU2Matrix|Generator[SU2Matrix, None, None]:
        if(generator):
    ...
    def __getitem__(self, nix:Iterable[int]|int)->SU2Matrix:
        return self(nix, generator = False)
    
    def __iter__(self , nix:None|int|Iterable[int])->Generator[SU2Matrix, None, None]:
        nix = self.check_index(nix)
        def yield_jw_trans(nix:Iterable[int], N:int, dtype:torch.dtype, create_or_annhilate:str, device:str|int)->Generator[SU2Matrix, None, None]:
            ...
            for i in nix:
                ...
                basis[0, :, 0] = 1
                yield SU2Matrix(basis, torch.tensor([complex(1, 0)], dtype= dtype, device=device))
        
        return yield_jw_trans(nix, self.N, dtype = self.dtype, device = self.device, create_or_annhilate=self.create_or_annhilate)

    def __pauli__(self, nix:int|Iterable[int]|None = None)->SU2Matrix:
        if(nix is None):
            return self.__getall__()
        nix = self.check_index(nix)
        if(isinstance(nix, range)):
            nix = torch.arange(start = nix.start, stop=nix.stop, step = nix.step)
        return self.get_ix(nix=nix)
    
    def get_ix(self, nix:torch.Tensor|np.ndarray)->SU2Matrix:
        ...
        basis[[torch.arange(nix.shape[0]), nix]] = torch.tensor((0,1/2,v,-1),dtype = self.dtype, device = self.device)
        b = torch.zeros(4, dtype= self.dtype, device= self.device)
        b[3] = 1
        return SU2Matrix(basis = basis+b, coefs=torch.ones(basis.shape[0], dtype = self.dtype, device=self.device))
    ...
    def __getall__(self ):
        ...
        basis[:,:,3] = 1.0+0j
        basis += (
            torch.einsum('AB,i->ABi', torch.eye(self.N, dtype = self.dtype, device=self.device), torch.tensor((0,1/2,v, -1))) 
                +
        ...
        return SU2Matrix(basis, torch.ones(basis.shape[0], dtype = self.dtype, device=self.device))
def is_iter(a):
    try:
        iter(a)
        return True
