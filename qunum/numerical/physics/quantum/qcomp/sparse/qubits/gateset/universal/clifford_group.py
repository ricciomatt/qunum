...
def pauli(n_qubits:int, target:int|Iterable[int], *args:tuple, dir_:str='x', device:torch.device = 'cpu', dtype:torch.dtype = torch.complex128, stpIt:bool = True, **kwargs:dict)->Generator[SU2Matrix, None, None]:
    ...
    while True:
        ...
        Basis[0,:, 0] = 1
        Basis[0, target] = B
        yield SU2Matrix(Basis, coefs=torch.tensor([1.0+0j], dtype = dtype, device= device))
        if(stpIt):
            break

def rot( n_qubits:int, target:int|Iterable[int], theta:torch.Tensor|float, *args:tuple, dir_:str = 'x', device:torch.device = 'cpu', dtype:torch.dtype = torch.complex128, stpIt:bool = True, **kwargs)->Generator[SU2Matrix, None, None]:
    if not (isinstance(theta, torch.Tensor)):
...
def hadamard(n_qubits:int, target:int|Iterable[int], *args:tuple, device:torch.device = 'cpu', dtype:torch.dtype = torch.complex128, stpIt:bool = True, **kwargs)->Generator[SU2Matrix, None, None]:
...
def s_gate(n_qubits:int, target:int|Iterable[int], *args:tuple, device:torch.device = 'cpu', dtype:torch.dtype = torch.complex128, stpIt:bool = True, **kwargs)->Generator[SU2Matrix, None, None]:
...
def controlled_pauli(n_qubits:int, target:int|Iterable[int], control:int|Iterable[int],  *args:tuple, dir_:str='x', device:torch.device = 'cpu', dtype:torch.dtype = torch.complex128, stpIt:bool = True, **kwargs)->Generator[SU2Matrix, None, None]:
    ...
    while True:
        ...
        yield SU2Matrix(Basis, coefs = Coefs)
        if(stpIt):
            break

def controlled_rotation(target:int|Iterable[int], control:int|Iterable[int], n_qubits:int, theta:float|torch.Tensor, *args:tuple, device:torch.device = 'cpu', dtype:torch.dtype = torch.complex128, stpIt:bool = True, **kwargs)->Generator[SU2Matrix, None, None]:
    while True:
        Basis = torch.zeros((1,n_qubits, 4), dtype = dtype, device = device)
        yield SU2Matrix(Basis, coefs = torch.tensor([1+0j], dtype = dtype, device = device))
        if(stpIt):
