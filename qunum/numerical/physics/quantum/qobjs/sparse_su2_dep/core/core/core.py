...
def contractCoef(a:Tensor|LazyTensor, b:Tensor|LazyTensor, idx:Tensor, enum:EnumerateArgCombos, Projection:Tensor, dims:int|Iterable[int]=2, out:Tensor|None = None, **kwargs)->Tensor|LazyTensor:
    match (a,b):
        ...
        case (a,b) if isinstance(a, Tensor) and isinstance(b, Tensor):
            a = a[idx[:,0]]*b[idx[:,1]]
    return contractLazy(a, Projection, dims = dims, out=out)
