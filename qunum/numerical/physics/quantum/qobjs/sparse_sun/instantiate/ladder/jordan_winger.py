...
class LazyJordanWigner:
    ...
    def __call__(self, nix:Iterable[int]|int|None = None, generator:bool=False) -> SUNMatrix|Generator[SUNMatrix, None, None]:
        if(generator):
    ...
    def __getitem__(self, nix:Iterable[int]|int)->SUNMatrix:
        return self(nix, generator = False)
    
    def __iter__(self , nix:None|int|Iterable[int])->Generator[SUNMatrix, None, None]:
        nix = self.check_index(nix)
        def yield_jw_trans(nix:Iterable[int], N:int, dtype:torch.dtype, create_or_annhilate:str, device:str|int)->Generator[SUNMatrix, None, None]:
            ...
            for i in nix:
                ...
                basis[0, :, 3] = 1
                yield SUNMatrix(basis, torch.tensor([complex(1, 0)], dtype= dtype, device=device), N = 2)
        
        return yield_jw_trans(nix, self.N, dtype = self.dtype, device = self.device, create_or_annhilate=self.create_or_annhilate)

    def __pauli__(self, nix:int|Iterable[int]|None = None)->SUNMatrix:
        if(nix is None):
    ...
    def get_ix(self, nix:torch.Tensor|np.ndarray)->SUNMatrix:
        ...
        b[0] = 1
        return SUNMatrix(basis = basis+b, coefs=torch.ones(basis.shape[0], dtype = self.dtype, device=self.device))
    ...
    def __getall__(self ):
        ...
        return SUNMatrix(basis, torch.ones(basis.shape[0], dtype = self.dtype, device=self.device))
def is_iter(a):
    try:
        iter(a)
        return True
