...
class NormScaler:
    def __init__(self, df:pl.DataFrame, x_cols:np.ndarray|pl.col|Iterable|Sequence|list, min_:int = 1e-6, max_:int = 1):
        self.x_cols = pl.col(x_cols)
        self.min_ = min_
        self.max_ = max_
        self.scale_build(df)
        return
    ...
    def scale_build(self, df:pl.DataFrame)->None:
        ...
        """        
        self.max_scle_facts = df.with_columns(self.x_cols).max()
        self.min_scle_facts = df.with_columns(self.x_cols).min() 
